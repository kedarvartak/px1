...
- **Hit identity.** A hit's `key` is the rule id, path, and a hash of the trimmed line text, not its line number, so **Ignore here** (`dismissedRuleHits` in the session manifest) survives edits that move the line. Rule `hits` counts each key once per process.
