...
- **Starting the review.** An agent given its own worktree is usually finished before anyone opens px1, and a baseline snapshotted at that moment would record the finished work as the starting state. So `autoStartReview` ([`autoreview.go`](../../autoreview.go)) starts the session itself, at launch and after a switch, with `worktreeBase` — the commit checked out when the linked worktree was created, from its per-worktree HEAD reflog — as the baseline. This avoids pulling in changes that were already on a starting branch such as `staging`. `reviewManager.StartFromRef` materialises that commit through `git archive` instead of walking the working tree, so committed and uncommitted agent work both read as changes. Only linked worktrees are started this way: the main checkout holds unrelated work in progress, and calling that "agent changes" would be false. The setting `review.autoStart` turns it off.
