...
- **Starting the review.** `autoStartReview` and `autoStartWorktreeReviews` ([`autoreview.go`](../../autoreview.go)) start sessions at launch and after a switch, so users start px1 once and run any harness normally. The latter registers every linked checkout for the Review inbox before it is opened. Main checkouts and non-worktree directories snapshot their current working tree; existing local work is therefore baseline content and only subsequent harness changes enter the queue. An agent given its own linked worktree may already be finished before px1 opens it, so that case uses `worktreeBase` — the commit checked out when the worktree was created, from its per-worktree HEAD reflog — as the baseline. This avoids pulling in changes that were already on a starting branch such as `staging`. `reviewManager.StartFromRef` materialises that commit through `git archive`, so committed and uncommitted agent work both read as changes. The setting `review.autoStart` turns automatic baselines off.
