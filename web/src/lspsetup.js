...
function drawSetup(s, d) {
  ...
  if (!s.enabled) {
    return hintHtml('Language servers are turned off: px1 was started with <b>-no-lsp</b>. ' +
  ...
  if (!offer.length) {
    html += '<p>px1 has no installer for this one. Install ' + s.servers.map(v => '<b>' + esc(v.name) + '</b>').join(' or ') +
